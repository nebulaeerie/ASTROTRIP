...
function PostModal({ post, lang, onClose }) {
  ...
  return (
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, zIndex: 300, background: 'rgba(0,0,10,0.85)', backdropFilter: 'blur(12px)', display: 'flex', alignItems: 'flex-start', justifyContent: 'center', overflowY: 'auto', padding: '48px 24px' }}>
...
export default function PostsPage({ type, onBack }) {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#02030f', fontFamily: 'Inter, system-ui, sans-serif' }}>
      <header style={{ position: 'sticky', top: 0, zIndex: 100, display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px 48px', background: 'rgba(2,3,15,0.95)', backdropFilter: 'blur(20px)', borderBottom: '1px solid rgba(80,110,200,0.12)' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: '28px', padding: '0 48px 48px', maxWidth: '1280px', margin: '0 auto' }}>
